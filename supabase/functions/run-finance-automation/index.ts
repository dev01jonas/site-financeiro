...
function dashboardMainRange(column: string) {
  return `${quoteSheetName('Operação Atlas (PRD)')}!${column}2:${column}${DASHBOARD_DATA_LAST_ROW}`
}

function dashboardCurrencyArray(column: string) {
  const range = dashboardMainRange(column)
  return `ARRAYFORMULA(IFERROR(VALUE(SUBSTITUTE(SUBSTITUTE(SUBSTITUTE(TO_TEXT(${range}),"R$",""),".",""),",",".")),0))`
}

function dashboardCurrencySum(column: string) {
  return `=SUM(${dashboardCurrencyArray(column)})`
}

function dashboardCurrencySumIf(column: string, statusRegex: string) {
  return `=IFERROR(SUM(FILTER(${dashboardCurrencyArray(column)},REGEXMATCH(UPPER(${dashboardMainRange('O')}),"${statusRegex}"))),0)`
}

function dashboardCurrencySumByRegua(column: string, stageCell: string) {
  return `=IFERROR(SUM(FILTER(${dashboardCurrencyArray(column)},${dashboardMainRange('AA')}=${stageCell})),0)`
}

function dashboardCurrencySumByYear(column: string, yearCell: string) {
  return `=IFERROR(SUM(FILTER(${dashboardCurrencyArray(column)},YEAR(${dashboardMainRange('A')})=${yearCell})),0)`
}

function dashboardCountByStatus(statusRegex: string) {
  return `=COUNTIF(${dashboardMainRange('O')},"*${statusRegex}*")`
}
...
function dashboardPieChartSpec(sheetId: number) {
  return {
    ...
    pieChart: {
      ...
      domain: {
        sourceRange: {
          sources: [gridRange(sheetId, 10, 15, 1, 2)],
        },
      },
      series: {
        sourceRange: {
          sources: [gridRange(sheetId, 10, 15, 2, 3)],
        },
      },
    },
  }
...
function dashboardMonthlyLineChartSpec(sheetId: number) {
  return {
    title: 'Histórico mensal: em atraso x em dia',
    basicChart: {
      chartType: 'LINE',
      ...
      domains: [
        {
          domain: {
            sourceRange: {
              sources: [gridRange(sheetId, 47, 68, 1, 2)],
            },
          },
        },
      ],
      series: [
        {
          series: {
            sourceRange: {
              sources: [gridRange(sheetId, 47, 68, 2, 3)],
            },
          },
        ...
        },
        {
          series: {
            sourceRange: {
              sources: [gridRange(sheetId, 47, 68, 3, 4)],
            },
          },
          targetAxis: 'LEFT_AXIS',
        },
...
async function repairDashboard(
  ...
  const situationRows = [
    ['Em atraso', dashboardCountByStatus('ATRASO'), dashboardCurrencySumIf('U', 'ATRASO'), dashboardCurrencySumIf('V', 'ATRASO'), dashboardCurrencySumIf('W', 'ATRASO')],
    ['A vencer', '=COUNTIF(' + dashboardMainRange('O') + ',"*A VENCER*")', dashboardCurrencySumIf('U', 'A VENCER'), dashboardCurrencySumIf('V', 'A VENCER'), dashboardCurrencySumIf('W', 'A VENCER')],
    ['Pago / quitado', '=COUNTIF(' + dashboardMainRange('O') + ',"*QUITADO*")+COUNTIF(' + dashboardMainRange('O') + ',"*PAGO*")', dashboardCurrencySumIf('U', 'QUITADO|PAGO'), dashboardCurrencySumIf('V', 'QUITADO|PAGO'), dashboardCurrencySumIf('W', 'QUITADO|PAGO')],
    ['Rescindido', '=COUNTIF(' + dashboardMainRange('O') + ',"*RESCIND*")', dashboardCurrencySumIf('U', 'RESCIND'), dashboardCurrencySumIf('V', 'RESCIND'), dashboardCurrencySumIf('W', 'RESCIND')],
  ...
  const yearRows = ['2022', '2023', '2024', '2025', '2026'].map((year, index) => {
    ...
    return [
      year,
      dashboardCurrencySumByYear('U', `H${rowNumber}`),
      dashboardCurrencySumByYear('V', `H${rowNumber}`),
      dashboardCurrencySumByYear('W', `H${rowNumber}`),
      `=IFERROR(COUNTA(FILTER(${dashboardMainRange('I')},YEAR(${dashboardMainRange('A')})=H${rowNumber},REGEXMATCH(UPPER(${dashboardMainRange('O')}),"ATRASO"))),0)`,
    ]
  })
  ...
  const reguaRows = reguaLabels.map((label, index) => {
    ...
    const labelCondition = label === 'Sem régua'
      ? `((${dashboardMainRange('AA')}="")+(${dashboardMainRange('AA')}="-"))>0`
      : `${dashboardMainRange('AA')}=B${rowNumber}`
    const countFormula = label === 'Sem régua'
      ? `=IFERROR(COUNTA(FILTER(${dashboardMainRange('I')},${dashboardMainRange('I')}<>"",${labelCondition})),0)`
      : `=COUNTIF(${dashboardMainRange('AA')},B${rowNumber})`
    const activeFormula = label === 'Sem régua'
      ? `=IFERROR(COUNTA(FILTER(${dashboardMainRange('I')},${dashboardMainRange('X')}="ATIVO",${labelCondition})),0)`
      : `=COUNTIFS(${dashboardMainRange('AA')},B${rowNumber},${dashboardMainRange('X')},"ATIVO")`
    const inactiveFormula = label === 'Sem régua'
      ? `=IFERROR(COUNTA(FILTER(${dashboardMainRange('I')},${dashboardMainRange('X')}="INATIVO",${labelCondition})),0)`
      : `=COUNTIFS(${dashboardMainRange('AA')},B${rowNumber},${dashboardMainRange('X')},"INATIVO")`
    const amountFormula = label === 'Sem régua'
      ? `=IFERROR(SUM(FILTER(${dashboardCurrencyArray('U')},${labelCondition})),0)`
      : dashboardCurrencySumByRegua('U', `B${rowNumber}`)

    return [label, countFormula, activeFormula, inactiveFormula, amountFormula]
  })

  const monthlyRows = Array.from({ length: 20 }, (_, index) => {
    const rowNumber = 49 + index
    const monthCell = `B${rowNumber}`
    const monthListFormula = `SORT(UNIQUE(FILTER(TEXT(${dashboardMainRange('A')},"yyyy/mm"),${dashboardMainRange('I')}<>"",${dashboardMainRange('A')}<>"")),1,FALSE)`
    return [
      `=IFERROR(INDEX(${monthListFormula},${index + 1}),"")`,
      `=IF(${monthCell}="","",IFERROR(COUNTA(FILTER(${dashboardMainRange('I')},TEXT(${dashboardMainRange('A')},"yyyy/mm")=${monthCell},REGEXMATCH(UPPER(${dashboardMainRange('O')}),"ATRASO"))),0))`,
      `=IF(${monthCell}="","",IFERROR(COUNTA(FILTER(${dashboardMainRange('I')},TEXT(${dashboardMainRange('A')},"yyyy/mm")=${monthCell},REGEXMATCH(UPPER(${dashboardMainRange('O')}),"EM DIA|A VENCER|PENDENTE"))),0))`,
  ...
  const weeklyRows = [
    ...
    return [
      label,
      `=COUNTIF(${dashboardMainRange('AA')},"${stageLabel}")`,
    ...
    await sheets.batchUpdateValues([
      {
        ...
        values: [[
          `=COUNTA(${dashboardMainRange('I')})`,
          '',
          dashboardCountByStatus('ATRASO'),
          '',
          dashboardCurrencySum('U'),
          '',
          dashboardCountByStatus('QUITADO') + '+COUNTIF(' + dashboardMainRange('O') + ',"*PAGO*")',
          '',
          dashboardCurrencySum('W'),
          '',
          '=IFERROR(MAX(FILTER(' + dashboardMainRange('AF') + ',' + dashboardMainRange('AF') + '<>"")),"")',
          '',
          '',
        ]],
      },
      ...
      },
      {
        ...
        values: [
          ['Indicador', 'Valor'],
          ['Sem atualização hoje', `=COUNTIFS(${dashboardMainRange('AF')},">"&TODAY(),${dashboardMainRange('AF')},"<"&TODAY()+1)`],
          ['Clientes sem régua', '=C20'],
          ['Em atraso sem ação', `=COUNTIFS(${dashboardMainRange('O')},"*ATRASO*",${dashboardMainRange('AA')},"") + COUNTIFS(${dashboardMainRange('O')},"*ATRASO*",${dashboardMainRange('AA')},"-")`],
          ['Dias vencido médio', `=IFERROR(AVERAGE(FILTER(${dashboardMainRange('Y')},${dashboardMainRange('Y')}>0)),0)`],
          ['Maior atraso', `=IFERROR(MAX(${dashboardMainRange('Y')}),0)`],
        ],
      },
      {
        range: `${dashboardSheet}!B49:E68`,
        values: monthlyRows,
      },
      {
        range: `${dashboardSheet}!H49:K55`,
        values: weeklyRows,
      },
    ])
        ...
        .map((chart) => ({ deleteEmbeddedObject: { objectId: chart.chartId } })),
      {
        addChart: {
          chart: {
            ...
            position: {
              overlayPosition: {
                anchorCell: { sheetId: dashboardProperties.sheetId, rowIndex: 37, columnIndex: 6 },
                offsetXPixels: 10,
                offsetYPixels: 8,
                widthPixels: 440,
                heightPixels: 260,
    ...
    preview: [
      {
        ...
        action: dryRun ? 'Prévia do dashboard' : 'Dashboard corrigido',
